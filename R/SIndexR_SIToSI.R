...
#' @description
#'  Site index conversion between species when a source-target mapping is defined.
...
SIndexR_SIToSI <- function(sp_index1, site, sp_index2){
  ...
  sp_index2 <- wholeToInteger(sp_index2, "sp_index2")
  ...
  site2 <- unlist(lapply(inputdata_list, function(s) SI2SI(s[[1]],
                                                            s[[2]],
                                                            s[[3]])))
  error <- site2
  error[error > 0] <- 0
